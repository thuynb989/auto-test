...
def search_result_page_verify_the_number_of_items_is_in_each_page(context: Context, number: str) -> None:
    ...
    for current_page in range(number_page-1):
        ...
        logger.info(f'number_item {number_item}')
        assert_that(number_item, equal_to(int(number)))
...
def verify_the_result_is_sorted_by_publication_date(context: Context) -> None:
    ...
    for element in search_result_page.get_list_publication_date_in_result_list():
        date = datetime.strptime(element.text, "%b %d, %Y").date()
        dates.append(date)
